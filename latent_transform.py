...
def train_eval(G, data_geo_z, data_tex_z, text_prompt, n_epochs=5, lmbda_1=0.0015, lmbda_2=0.0015, edit_geo=True, edit_tex=True):
    ...
    clip_loss = CLIPLoss('car', text_prompt)
    ...
    # Train networks to produce original latent
    for i in range(100):
        ...
        loss_geo = ((geo_z_edited - geo_z)**2).sum()
        loss_tex = ((tex_z_edited - tex_z)**2).sum()
    ...
    print("Network Weight Initialisation Complete")
    ...
    for i in range(n_epochs):
        ...
        c = torch.ones(1, device='cuda')
        output = eval_get3d_weights(g_ema, geo_z_edited, tex_z_edited, c)

        cur_output = output.detach()

        # Get CLIP Loss
        loss_clip = clip_loss.global_loss(output) # + clip_loss(cur_output, img_original)
...
if __name__ == "__main__":
    ...
    original, edited, loss, min_latent, edited_images = train_eval(G_ema, z, tex_z, text_prompt, n_epochs=n_epochs, lmbda_1=lmbda_1, lmbda_2=lmbda_2)
