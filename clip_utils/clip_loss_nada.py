...
from torch.nn.functional import cosine_similarity, cross_entropy, normalize

@torch.no_grad()
def get_pae(model, args, image_features, text_features, all_texts):
    semantic_basis_text = all_texts
    subspace_basis = clip.tokenize(semantic_basis_text).to('cuda')
    ...
    for i, text_feature in enumerate(text_features):
        for j, image_feature in enumerate(image_features):
            ...
            if "+" in args['target']:
                ...
                target += (shift / text_coeff_sum[i]) * (text_feature @ subspace_basis.T) @ subspace_basis
            else: # pae
...
class CLIPLoss(torch.nn.Module):
    ...
    def __init__(self, source_text, target_text, aux_string='', corpus=[], method='All', original_images=None):
        ...
        with torch.no_grad():
            ...
            self.cos_sim = torch.nn.CosineSimilarity(eps=1e-11)
            ...
            if corpus:
                ...
                self.corpus_tokenized = clip.tokenize([f'{aux_string}{word.lower()}' for word in corpus]).to('cuda')
                ...
                self.projection_target = torch.tensor([0], device='cuda')
                # self.text_projected = (normalize(self.target_text_encoded) @ self.components.T).softmax(dim=-1)
                # print(self.text_projected)
                self.original_images_embeddings = self.model.encode_image(self.preprocess(original_images))
                self.pae_targets = get_pae(self.model, {'power': 9, 'target': ''}, self.original_images_embeddings, (self.target_text_encoded), corpus)

    def projection_embedding_loss(self, image):
        # Get image embedding
        image_processed = self.preprocess(image)
        image_embeddings = normalize(self.model.encode_image(image_processed))

        # Project image embeddings onto basis
        similarity_scores = (image_embeddings @ self.components.T).softmax(dim=-1)
        # print(similarity_scores)

        # Get loss between this and the projected text
        #text_repeated = self.text_projected.repeat(image_projected.shape[0], 1)
        return cross_entropy(image_embeddings, self.projection_target.repeat(image_embeddings.shape[0]))
    ...
    def forward(self, source_image, target_image=None):
        ...
        return 1. - self.cos_sim(img_direction, self.direction)
