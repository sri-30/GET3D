...
from get3d_utils import constructGenerator, eval_get3d_single, eval_get3d_weights, eval_get3d_weights_ws, eval_nada, determine_opt_layers, unfreeze_generator_layers, freeze_generator_layers
...
def train_eval(G, data_geo_z, data_tex_z, text_prompt, n_epochs=5, lmbda_1=0.0015, lmbda_2=0.0015, edit_geo=True, edit_tex=True):
    ...
    clip_loss = CLIPLoss(source_text='a car', target_text=text_prompt)
    res_loss = []
    edited_images = []
    ...
    for i in range(n_epochs):
        ...
        ws_geo = g_ema_frozen.mapping_geo(z_geo, 0, update_emas=False)
        ws_tex = g_ema_frozen.mapping(z_tex, 0, update_emas=False)

        # Get output of GET3D on latents

        with torch.no_grad():
            frozen_img = eval_nada(g_ema_frozen, g_ema_frozen,ws=ws_tex, ws_geo=ws_geo, n_cameras=4)

        trainable_img = eval_nada(g_ema_train, g_ema_frozen,ws=ws_tex, ws_geo=ws_geo, n_cameras=4)
