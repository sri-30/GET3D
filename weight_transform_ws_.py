...
def train_eval(G, data_geo_z, data_tex_z, text_prompt, n_epochs=5, lmbda_1=0.0015, lmbda_2=0.0015, edit_geo=True, edit_tex=True):
    ...
    with torch.no_grad():
        ...
        data_ws = g_ema_frozen.mapping(tex_z, grid_c, truncation_psi=0.7, update_emas=False)
        data_ws_geo = g_ema_frozen.mapping_geo(
            geo_z, grid_c,
            truncation_psi=0.7, update_emas=False)
    ...
    clip_loss = CLIPLoss(source_text='car', target_text=text_prompt, aux_string='a photo of a')
    res_loss = []
    edited_images = []
    camera_idx=[4, 8, 12]
    ...
    freeze_generator_layers(g_ema_frozen)
    ...
    for i in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            ws_geo = g_ema_frozen.mapping_geo(z_geo, 0, truncation_psi=0.7, update_emas=False)
            ws_tex = g_ema_frozen.mapping(z_tex, 0, truncation_psi=0.7, update_emas=False)
        ...
        with torch.no_grad():
            frozen_img = eval_get3d_angles(g_ema_frozen, ws_geo, ws_tex, camera_idx=camera_idx, intermediate_space=True)

        trainable_img = eval_get3d_angles(g_ema_train, ws_geo, ws_tex, camera_idx=camera_idx, intermediate_space=True)
        ...
        with torch.no_grad():
            ...
            loss_ = clip_loss(img_original, img_save)
        ...
        # output = eval_get3d_weights_ws(g_ema_train, ws_geo, ws, c)
        # with torch.no_grad():
        #     img_original = eval_get3d_weights_ws(g_ema_frozen, ws_geo, ws, c)

        # cur_output = output.detach()

        # # Get CLIP Loss
        # loss_clip = clip_loss(output[0], original_image=img_original)
        ...
        # with torch.no_grad():
        #     if loss_clip.item() < min_loss:
        #         min_loss = loss_clip.item()

        #     res_loss.append((loss_clip.item(), 0, 0))
    
        # if i % n_save == 0:
        #     with torch.no_grad():
        #         img_save = eval_get3d_weights_ws(g_ema_train, data_ws_geo, data_ws, c)
        #         edited_images.append(img_save.cpu())
